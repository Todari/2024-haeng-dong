...
describe('Flow: 비회원이 랜딩 페이지에서부터 이벤트를 생성 완료하는 flow', () => {
  ...
  it('로그인 페이지에서 "뒤로가기" 버튼을 누르면 랜딩 페이지로 이동해야 한다.', () => {
    cy.visit(ROUTER_URLS.landing);
    cy.get('button').contains('정산 시작하기').click();
    cy.get('button').contains('뒤로가기').click();
    cy.url().should('eq', `${Cypress.config().baseUrl}${ROUTER_URLS.landing}`);
  });
  ...
  // TODO: (@Todari) 에러 처리 테스트 추가
  // context('postEvent API 에러 처리', () => {
  //   beforeEach(() => {
  //     cy.createAdminName(CONSTANTS.eventName, CONSTANTS.adminName);
  //     cy.get('input').type(CONSTANTS.eventPassword);
  //   });

  //   it('이벤트 생성 API가 실패하면 에러 메시지를 표시해야 한다', () => {
  //     cy.interceptAPI({type: 'postEvent', statusCode: 400});
  //     cy.get('button').contains('행동 개시!').click();
  //     cy.contains('이벤트 생성에 실패했습니다').should('be.visible');
  //   });

  //   it('네트워크 오류 발생 시 재시도 버튼이 표시되어야 한다', () => {
  //     cy.interceptAPI({type: 'postEvent', statusCode: 500});
  //     cy.get('button').contains('행동 개시!').click();
